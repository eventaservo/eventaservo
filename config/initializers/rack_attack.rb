...
class Rack::Attack
  ...
  throttle("requests by subnet /24", limit: 30, period: 10) do |req|
    req.ip.to_s.split(".")[0, 3].join(".") if req.ip.present?
  end
  ...
  throttle("requests by subnet /16", limit: 100, period: 60) do |req|
    req.ip.to_s.split(".")[0, 2].join(".") if req.ip.present?
  end
  ...
  throttle("continent pages by IP", limit: 3, period: 10) do |req|
    req.ip if req.get? && CONTINENT_PATHS.include?(req.path)
  end
  ...
  throttle("limit API requests per IP", limit: 2, period: 10) do |req|
    req.ip if req.path == "/api/v2/organizations"
  end
...
ActiveSupport::Notifications.subscribe("throttle.rack_attack") do |name, start, finish, request_id, payload|
  req = payload[:request]
  Rails.logger.warn "[Rack::Attack] Throttled #{req.ip} - #{req.path} - #{payload[:match_type]}: #{payload[:match_data][:discriminator]}"
end
