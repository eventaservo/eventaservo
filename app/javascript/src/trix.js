  ...
  () => {
    if (document.querySelector('.trix-button-row')) {
      ...
      for (const [key, value] of Object.entries(atribuoj)) {
        document
          .querySelector(`.trix-button--icon-${key}`)
      ...
      ligilajKampoj
        .querySelector('.trix-input--dialog')
        .setAttribute('placeholder', 'Entajpu ligilon…')
      ligilajKampoj
        .querySelector("[data-trix-method='setAttribute']")
        .setAttribute('value', 'Ligi')
      ligilajKampoj
        .querySelector("[data-trix-method='removeAttribute']")
        .setAttribute('value', 'Malligi')
    }
  },
  false
)
